...
const UI_TRANSLATIONS: Record<string, Dict> = {
  ...
  'plag.dropTitle': { es: 'Arrastra tu archivo aquí (.docx, .pdf, .txt)', en: 'Drag your file here (.docx, .pdf, .txt)', pt: 'Arraste seu arquivo aqui (.docx, .pdf, .txt)', fr: 'Glissez votre fichier ici (.docx, .pdf, .txt)', it: 'Trascina qui il tuo file (.docx, .pdf, .txt)' },
  ...
  'plag.errorFormat': { es: 'Formato no admitido ({ext}). Usa PDF, DOCX, DOC, RTF, TXT o MD.', en: 'Unsupported format ({ext}). Use PDF, DOCX, DOC, RTF, TXT or MD.', pt: 'Formato não suportado ({ext}). Use PDF, DOCX, DOC, RTF, TXT ou MD.', fr: 'Format non pris en charge ({ext}). Utilisez PDF, DOCX, DOC, RTF, TXT ou MD.', it: 'Formato non supportato ({ext}). Usa PDF, DOCX, DOC, RTF, TXT o MD.' },
  ...
  'plag.overlayIncluded': { es: 'Cada informe adjunta solo las hojas marcadas de tu documento original sobre su propio formato (carátula, encabezados y paginación intactos) y cierra con la recomendación para corregir lo detectado.', en: 'Each report attaches only the marked pages of your original document over its own formatting (cover, headers and pagination intact) and closes with the recommendation for fixing what was found.', pt: 'Cada relatório anexa apenas as páginas marcadas do seu documento original sobre a própria formatação (capa, cabeçalhos e paginação intactos) e termina com a recomendação para corrigir o que foi detectado.', fr: 'Chaque rapport joint uniquement les pages marquées de votre document original sur sa propre mise en forme (page de garde, en-têtes et pagination intacts) et se termine par la recommandation pour corriger ce qui a été détecté.', it: 'Ogni rapporto allega solo le pagine marcate del tuo documento originale sulla propria formattazione (copertina, intestazioni e impaginazione intatte) e termina con la raccomandazione per correggere quanto rilevato.' },
  'plag.overlayUnavailable': { es: 'Sube el documento en PDF para que los informes lo adjunten marcado sobre su formato original. Con texto pegado o .docx no hay coordenadas de página y el informe sale reimpreso.', en: 'Upload the document as a PDF so the reports can attach it marked up in its original formatting. Pasted text or .docx carry no page coordinates, so the report is reprinted instead.', pt: 'Envie o documento em PDF para que os relatórios o anexem marcado sobre o formato original. Com texto colado ou .docx não há coordenadas de página e o relatório sai reimpresso.', fr: 'Téléversez le document en PDF pour que les rapports le joignent annoté dans sa mise en forme d\'origine. Un texte collé ou un .docx ne portent aucune coordonnée de page : le rapport est alors réimprimé.', it: 'Carica il documento in PDF affinché i rapporti lo allegino annotato nel formato originale. Con testo incollato o .docx non ci sono coordinate di pagina e il rapporto viene ristampato.' },
